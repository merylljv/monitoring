...
def shift_divider(key, year, month, next_start, shift_name, recompute=False):
    ...
    try:
        ...
        div_shift = shift_name not in allsheet.keys()
    except:
        div_shift = True
    ...
    if div_shift or recompute:
        ...
        shift_count['IOMP-MT'] = 1
        shift_count['IOMP-CT'] = 1
        shift_count.loc[shift_count.team == 'admin', 'IOMP-MT'] = 0
        shift_count.loc[shift_count.team.isin(['admin', 'lrap']), 'IOMP-CT'] += 1

        # check how many more shifts are needed
        rem_MT = 2 * num_days - shift_count['IOMP-MT'].sum()
        rem_CT = 2 * num_days - shift_count['IOMP-CT'].sum()

        # distribute remaining CT shifts to those with least CT so far
        for _ in range(int(rem_CT)):
            CT_candidates = shift_count[shift_count.team == 'CT'].copy()
            CT_candidates = CT_candidates.sort_values('IOMP-CT')
            name_to_update = CT_candidates.iloc[0]['name']
            shift_count.loc[shift_count.name == name_to_update, 'IOMP-CT'] += 1

        # distribute remaining MT shifts
        for _ in range(int(rem_MT)):
            MT_candidates = shift_count[shift_count.team == 'MT'].copy()
            MT_candidates = MT_candidates.sort_values('IOMP-MT')
            name_to_update = MT_candidates.iloc[0]['name']
            shift_count.loc[shift_count.name == name_to_update, 'IOMP-MT'] += 1
        ...
        # check total shifts
        print("Shift count processed")
        print("Total MT:", shift_count['IOMP-MT'].sum())
        print("Total CT:", shift_count['IOMP-CT'].sum())
...
def assign_remaining_IOMP(shiftdf, shift_count):
    ...
    remaining_IOMP = sorted(
        set(shift_count.loc[(shift_count['IOMP-MT'] != 0) & (shift_count['IOMP-CT'] != 0), 'name'].values)
        - set(extra_CT)
        - set(extra_MT)
    )
    ...
    # assign extra MT shifts
    temp_MT_shift = MT_shift[0:len(extra_MT)]
    MT_shift = sorted(set(MT_shift) - set(temp_MT_shift))
    for ts, name in zip(temp_MT_shift, extra_MT):
        shiftdf.loc[shiftdf.ts == ts, 'IOMP-MT'] = name

    # assign extra CT shifts
    temp_CT_shift = CT_shift[0:len(extra_CT)]
    CT_shift = sorted(set(CT_shift) - set(temp_CT_shift))
    for ts, name in zip(temp_CT_shift, extra_CT):
        shiftdf.loc[shiftdf.ts == ts, 'IOMP-CT'] = name
    ...
    for name in shift_count.name:
        ...
        if len(week_list) != len(set(week_list)):
            ...
            if any(df.groupby('week')['shift_count'].agg('sum') > 4):
                ts_list_str = pd.to_datetime(ts_list).strftime('%Y-%m-%d %H:%M').tolist()
                print(name, ': same week!', ts_list_str)
